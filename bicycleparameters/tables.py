...
class Table():
    ...
    def generate_variable_list(self):
        ...
        try:
            for bicycle in self.bicycles:
                allVariables += list(bicycle.parameters[self.source].keys())
        except TypeError:
            allVariables += list(self.bicycle.parameters[self.source].keys())
    ...
    def create_rst_table(self, fileName=None):
        ...
        for row in table:
            ...
            for i, pair in enumerate(list(zip(colSize, largest))):
                if pair[0] > pair[1]:
                    largest[i] = pair[0]
